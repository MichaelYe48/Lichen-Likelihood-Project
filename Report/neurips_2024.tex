...
Appendix B, Figure 3 depicts the graph structure of the Bayesian Network with Complete Data. Field Collection Date and Region serve as root nodes that connect directly to Pollution and Lichen Species. These in turn affect the quantities of different elements within lichen tissue samples.
...
The equations for Pollution $P(Pe \mid R, F)$, Lichen Species $P(Sp \mid F, Pe)$ and Tissue Composition $P(E \mid Sp, Pe)$ are as follows. $E$ is used to represent each element node for tissue composition \{$TNi,\ TS,\ TP,\ TPb,\ TCu,\ TCr$\}.
%
...
Appendix C, Figure 4 shows the updated belief network including the hidden node. This node separates our region/environment variables from the lichen species and air pollution score, but all other nodes are the same.
...
\right]
\]
Once we have learned our CPTs, we want to determine the relationship between the pollution levels and tissue data from our lichen. We do this using our CPTs to perform exact inference and calculate \(    P(\mathrm{Pe}=p \mid \vec{E}=\vec{e},\ \mathrm{Sp}=s) = \)\:
\[
    \frac{
        \displaystyle
        \sum_{h',r',f'}
...
Taking a look at the CPTs learned from our MLE model (Appendix F Table 4), we can quickly observe that region 10 historically has much better air quality than region 6.
This aligns with our intuition as region 10 represents Alaska, which is known to be one of the areas with the lowest air pollution in the US due to its low population density whereas region 6 represents the Pacific Northwest which has a higher population density and more industrial activity.
...
As a result, our choices of configurations are limited since adjusting configurations for the MLE model would require shifting away from the network structure we believe is ideal for capturing the node relationships of our dataset. Similarly, beyond changing the network structure for the EM model, we are limited to only adjusting the initialization values of our model.\newline\newline
As described above, our implementation of EM was able to consistently converge in roughly 200 iterations. This can be visibly seen in the plot of log likelihood in the Appendix. We also attempted to vary the number of hidden nodes in our belief network to test if increasing the number of learnable parameters(scaling up) would give us a better accuracy. However, training with $\{2,3,5\}$ hidden nodes did not result in an increase in accuracy. With regard to computational cost, EM performs linearly with respect to the size of our dataset, meaning that this approach would likely be feasible to scale up. Increasing the amount of data should allow for our learned CPTs to be more precise and generalized. Furthermore, introducing more sample diversity to our data should potentially make scaling up our belief network a more viable strategy. This is because a larger dataset would potentially contain more complex trends requiring more hidden variables to model effectively.
\newline

\section{Conclusion}

As stated in the previous section, our best-performing configuration is the EM-trained Bayesian network with a hidden node, which achieves about $72.4\%$ test accuracy when predicting pollution from region, field collection date, lichen species, and tissue chemistry, $P(P_e \mid R, F, Sp, T)$, compared to $33.6\%$ for a median-pollution baseline and $52.2\%$ for a simpler MLE model that predicts $P(P_e \mid Sp, T)$ from species and tissue alone.
Moreover conceptually, our results show that spatial context dominates the signal: the hidden node learned by EM essentially splits the data into two latent regimes aligned with Alaska vs.\ the Pacific Northwest, and its state largely determines whether pollution is low or moderate/high, while species identity and tissue element concentrations mainly refine predictions within a region.
...
Our advice for future groups is to make sure that you have a clear communication with the TA and to always turn in something for the milestones in order to ensure you get some form of feedback regardless of the progress.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\linewidth]{species.png}
    \label{Species Count}
    \caption{Samples per Species}
\end{figure}
\FloatBarrier

\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\linewidth]{apc_6_cat.png}
    \label{Air Pollution Count}
    \caption{6 Binned Air Pollution Count}
\end{figure}
\FloatBarrier
...
\begin{table}[h!]
    \centering
    \caption{Conditional Probability Table $P(\text{Phosphorus (binned)} \mid \text{Pollution, Species})$}
