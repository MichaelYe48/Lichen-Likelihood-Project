...
\vspace{-2ex}
The E and M steps are repeated in turn for every value of every node until the log-likelihood of the data converges or a maximum number of iterations is reached. As a reminder, we compute our log likelihood as: 
\[
\log L
= 
\sum_{i=1}^{N}
...
P(Pe_i \mid h)\;
P(sp_i \mid h, Pe_i)\;
\prod_{t\in\mathcal{T}} P(t_{ik} \mid Pe_i, sp_i)
\right]
\]
Once we have learned our CPTs, we want to determine a relationship between the element analysis and tissue data from our lichen. We do this using our CPTs to calculate: 
\[
P(\mathrm{Pe}=pe \mid \text{T=t},\, \mathrm{Sp=sp})
=
\frac{
\displaystyle
\sum_{h}
P(H=h)\,
P(\mathrm{Pe}=pe \mid H=h)\,
\prod_{t \in \mathcal{T}}
P\!\left(t=t \mid \mathrm{Pe}=pe,\; \mathrm{sp}\right)
}{
\displaystyle\sum_{pe'}
\sum_{h}
P(H=h)\,
P(\mathrm{Pe}=pe' \mid H=h)\,
\prod_{t \in \mathcal{T}}
