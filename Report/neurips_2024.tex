...
Since the network structure and CPTs are defined over a finite number of discrete states, all continuous variables were converted into categorical values. Due to the relatively small amount of data, air pollution score and tissue element concentrations were split into three categories (low, medium, high) based on percentiles with $\frac{1}{3}$ of the data categorized into each label. The year a sample was collected was also split into three groups, while region relied on previously defined regions given in the original data.
...
\begin{table}[h!]
    ...
    \begin{tabular}{| m{3cm} | m{3.5cm} | m{7cm} |}
        ...
        \hline
        Air pollution score & \makecell[l]{Low ($<^-0.46$)\\Medium($^-0.46- ^-0.14$)\\High ($> -0.139$)} & This is a categorized score of ambient air quality, often based on proximity to major industrial or traffic sources. The lower bound of the "High" category roughly corresponds to USFS's threshold for if an area is affected by pollution (-0.11 and above). The split between "Low" and "Medium" categories does not directly correspond to a USFS pollution threshold, but can be thought of as "Excellent" and "Good", respectively.\\
        \hline
