...
In the Expectation Step, we compute the posterior distribution of the hidden variable $H$ given the visible nodes {$F,\ R,\ Pe\ Sp$}. For a value of h $H=h$, we define:
...
In the Maximization Step, we update all CPTs using the expected counts from the E-step. For all equations described, the following notation holds true:\\
\vspace{-4ex}
\begin{itemize}
...
The tissue composition nodes are separated D from the hidden node, and therefore can follow the standard MLE formula. It is calculated with the following equation, where $X$ represents each respective node:
